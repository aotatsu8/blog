...
worker/      Cloudflare Worker（/go/:id リダイレクト。それ以外は out/ を静的配信）
...
本サイトは **Worker（[`worker/index.ts`](worker/index.ts)）** が `/go/:id` のリダイレクトを処理し、
それ以外は**静的書き出し（`out/`）をアセットとして配信**します。

| 設定項目 | 値 |
|---|---|
| Build command | `pnpm build` |
| Deploy command | `npx wrangler deploy` |
| 配信内容 | [`wrangler.jsonc`](wrangler.jsonc)：`main = worker/index.ts` ＋ `assets.directory = ./out` |
...
[`worker/index.ts`](worker/index.ts) の `LINKS` に `id → アフィリエイトURL` を登録すると、
記事側は内部URL `/go/<id>` を指定でき（例: `<ProductCard rakuten="/go/mx-mechanical-mini" />`）、
実リンクの差し替え・集約はこの `LINKS` を直すだけで全記事に反映できます。

## TODO（将来対応）

- **Cookie 同意バナー** — 未実装。Google AdSense 併用や EU 対応が必要になったら追加。
- **記事ごとの動的 OGP 画像** — 未対応（現状は frontmatter 指定の静的画像 / サイト共通デフォルト `public/og-default.png`）。
- **/go のサーバー側クリック計測** — リダイレクトは実装済み。記録（Analytics Engine / GA4 Measurement Protocol）は未実装（`worker/index.ts` の TODO 参照）。
